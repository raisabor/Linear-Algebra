...
class LinearSystem(object):
    ...
    def multiply_coefficient_and_row(self, coefficient, row):
        ...
        newNormalVector = n.timesScalar(coefficient)
        newConstantTerm = k * coefficient

        self[row] = Plane(normal_vector= newNormalVector, constant_term= newConstantTerm)
    ...
    def add_multiple_times_row_to_row(self, coefficient, row_to_add, row_to_be_added_to):
        ...
        newNormalVector = n1.timesScalar(coefficient).plus(n2)
        newConstantTerm = (k1 * coefficient) + k2

        self[row_to_be_added_to] = Plane(normal_vector=newNormalVector, constant_term=newConstantTerm)
    ...
    def computer_triangular_form(self):
        system = deepcopy(self)

        numEquations = len(system)
    ...
    def computer_rref(self):
        tf = self.computer_triangular_form()
        ...
        for i in range(num_equations)[::-1]:
            ...
            if j < 0:
                continue
            tf.swap_with_row_to_make_coefficient_equal_one(i, j)
            tf.clear_coefficients_above(i, j)

        return tf


    def swap_with_row_to_make_coefficient_equal_one(self, row, col):
        n = self[row].normal_vector
